...
export default function HomeContent() {
  ...
  const [deviceTilt, setDeviceTilt] = useState({ x: 0.5, y: 0.5 });
  ...
  // Gyroscope spotlight for mobile
  useEffect(() => {
    ...
    const handleOrientation = (e: DeviceOrientationEvent) => {
      ...
      const beta  = Math.max(-20, Math.min(50, e.beta));
      setDeviceTilt({
        x: (gamma + 40) / 80,
        y: (beta  + 20) / 70,
      });
    ...
    const startListening = () => {
      ...
      removeListener = () => window.removeEventListener("deviceorientation", handleOrientation, true);
    };

    // iOS 13.3+ requires explicit permission
    // Use window.DeviceOrientationEvent to avoid ReferenceError in browsers where it's undefined
    const DOE = (window as unknown as { DeviceOrientationEvent?: { requestPermission?: () => Promise<string> } }).DeviceOrientationEvent;
    if (DOE && typeof DOE.requestPermission === "function") {
      // Request on first touch — must be direct user gesture
      const onTouch = () => {
  ...
  // Helper: spotlight position string — px no desktop, % no mobile (giroscópio)
  const sp = (hover: boolean, x: number, y: number) => ({
    active: isMobile ? true : hover,
    pos: isMobile
      ? `${deviceTilt.x * 100}% ${deviceTilt.y * 100}%`
      : `${x}px ${y}px`,
  });
    ...
    <main className="relative min-h-[100dvh] flex flex-col select-none">
      ...
      >
        <motion.div
          ...
          onMouseLeave={() => setHeaderMouse((p) => ({ ...p, hover: false }))}
        >
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <motion.button
              ...
              onMouseLeave={() => setCtrlSpotlight((p) => ({ ...p, id: null }))}
            >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center text-center flex-1 px-5 pb-8 pt-4 sm:px-6 sm:pb-16 sm:pt-8 gap-6 sm:gap-10 md:gap-12">
        ...
        <motion.h1
          ...
          animate="visible"
          variants={fadeUp}
          className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black leading-[1.1] tracking-tight flex flex-wrap items-center justify-center gap-x-2 sm:gap-x-3 gap-y-2"
        ...
        >
          ...
          <RotatingText
            ...
            onMouseLeave={() => setPillSpotlight((p) => ({ ...p, hover: false }))}
        ...
        >
          <motion.a
            href="/software"
            className="relative w-full sm:w-auto px-6 py-2.5 sm:px-8 sm:py-3 rounded-full text-sm sm:text-base font-bold text-center overflow-hidden"
            ...
            onMouseLeave={() => setHeroSpotlight((p) => ({ ...p, id: null }))}
          >
          ...
          <motion.a
            href="/agency"
            className="relative w-full sm:w-auto px-6 py-2.5 sm:px-8 sm:py-3 rounded-full text-sm sm:text-base font-bold text-center"
