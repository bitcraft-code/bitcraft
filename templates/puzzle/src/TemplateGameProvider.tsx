...
export const TemplateGameProvider = ({ children }: { children: ReactNode }) => {
  const analytics = useMemo(
    () => createAnalyticsClient(createMockAnalyticsProvider("template")),
    [],
  );
  ...
  const storage = useMemo(() => createAsyncStorageAdapter("template"), []);
    ...
    const load = async () => {
      try {
        ...
        await analytics.trackEvent(
          createGameOpenedEvent("template", {
  ...
  useEffect(() => {
    ...
    void settingsStore.save(settings).catch((error) => {
      console.error("[template] saveSettings", error);
    });
  ...
  useEffect(() => {
    ...
    void highScoreStore.save(session.highScore).catch((error) => {
      console.error("[template] saveHighScore", error);
    });
  ...
  const updateSetting = <K extends keyof TemplateSettings>(
    ...
    setSettings((current) => {
      ...
      void analytics.trackEvent(
        createSettingsUpdatedEvent("template", {
          setting: String(key),
        }),
      );
  ...
  const previewPremiumOffer = async () => {
    ...
    await analytics.trackEvent(
      createPurchaseCompletedEvent("template", {
  ...
  const trackSetupEvent = async () => {
    await analytics.track("template.setup_event", {
