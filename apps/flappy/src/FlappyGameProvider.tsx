...
export const TemplateGameProvider = ({ children }: { children: ReactNode }) => {
  const analytics = useMemo(
    () => createAnalyticsClient(createMockAnalyticsProvider("flappy")),
    [],
  );
  ...
  const storage = useMemo(() => createAsyncStorageAdapter("flappy"), []);
    ...
    const load = async () => {
      try {
        ...
        await analytics.trackEvent(
          createGameOpenedEvent("flappy", {
            highScore: savedHighScore,
          }),
        );
      } catch (error) {
        console.error("[flappy] load", error);
      } finally {
        if (active) {
          setReady(true);
        }
  ...
  useEffect(() => {
    ...
    void settingsStore.save(settings).catch((error) => {
      console.error("[flappy] saveSettings", error);
    });
  ...
  useEffect(() => {
    ...
    void highScoreStore.save(session.highScore).catch((error) => {
      console.error("[flappy] saveHighScore", error);
    });
  ...
  const updateSetting = <K extends keyof TemplateSettings>(
    ...
    setSettings((current) => {
      ...
      void analytics.trackEvent(
        createSettingsUpdatedEvent("flappy", {
          setting: String(key),
        }),
      );
  ...
  const previewPremiumOffer = async () => {
    ...
    await analytics.trackEvent(
      createPurchaseCompletedEvent("flappy", {
  ...
  const trackSetupEvent = async () => {
    await analytics.track("flappy.setup_event", {
...
export const useTemplateGame = () => {
  const context = useContext(TemplateGameContext);

  if (!context) {
    throw new Error("useTemplateGame must be used inside TemplateGameProvider.");
  }
